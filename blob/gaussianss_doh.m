function SS = gaussianss_doh(I,min_scale,max_scale,S,sigma0,slow)
...
for i=1:length(SS.scales)
    ...
    if slow
        ...
        Lxx=subdxx(SS.octave(:,:,i),1);
        Lxy=subdxy(SS.octave(:,:,i),1);
        Lyy=subdyy(SS.octave(:,:,i),1);
    end
    SS.doH(:,:,i)= (Lxx.*Lyy-Lxy.*Lxy)*SS.scales(i)^4;
    SS.Lxx(:,:,i)= Lxx;
    SS.Lyy(:,:,i)= Lyy;
    SS.Lxy(:,:,i)= Lxy;
end
