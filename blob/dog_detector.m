function [oframes]=dog_detector(I,varargin)
...
gss = gaussianss_dog(I,min_scale,max_scale,S,sigma0) ;
%c=0.01,sqrt(K)=50
idx = siftlocalmax(  gss.doG, 0.00385208  ) ;
idx = [idx , siftlocalmax( - gss.doG, 0.00385208)] ;

[i,j,s] = ind2sub( size( gss.doG ), idx ) ;
oframes = [j(:)';i(:)';s(:)'] ;
oframes(1:3,:) = oframes(1:3,:) - 1;
oframes = dohrefinemx(oframes,gss.doG) ;
oframes(1:3,:) = oframes(1:3,:) + 1;
...
yr=cordr(2,:);
zr=cordr(3,:);
ind1=sub2ind(size(gss.doG),y1,x1,z1);
ind2=sub2ind(size(gss.doG),y1,x1,z2);
ind3=sub2ind(size(gss.doG),y2,x1,z1);
ind4=sub2ind(size(gss.doG),y2,x1,z2);
ind5=sub2ind(size(gss.doG),y1,x2,z1);
ind6=sub2ind(size(gss.doG),y1,x2,z2);
ind7=sub2ind(size(gss.doG),y2,x2,z1);
ind8=sub2ind(size(gss.doG),y2,x2,z2);
indr=sub2ind(size(gss.doG),yr,xr,zr); %20130606
x20=x2-x;
y20=y2-y;
z20=z2-z;
x01=x-x1;
...
doG=gss.doG(ind1) .* x20 .* y20 .* z20 ./ x21 ./ y21 ./ z21 + ...
    gss.doG(ind2) .* x20 .* y20 .* z01 ./ x21 ./ y21 ./ z21 + ...
    gss.doG(ind3) .* x20 .* y01 .* z20 ./ x21 ./ y21 ./ z21 + ...
    gss.doG(ind4) .* x20 .* y01 .* z01 ./ x21 ./ y21 ./ z21 + ...
    gss.doG(ind5) .* x01 .* y20 .* z20 ./ x21 ./ y21 ./ z21 + ...
    gss.doG(ind6) .* x01 .* y20 .* z01 ./ x21 ./ y21 ./ z21 + ...
    gss.doG(ind7) .* x01 .* y01 .* z20 ./ x21 ./ y21 ./ z21 + ...
    gss.doG(ind8) .* x01 .* y01 .* z01 ./ x21 ./ y21 ./ z21;
...
K = (r + 3 * r .^ 3) ./ (3 + r .^ 2);
H = ((3 + r .^ 2) ./ r ./ (1 + r)) / 0.2e1;
c =   -(doG.* (3 + 2 * r + 3 * r .^ 2) .^ 2 ./ (1 + r) .^ 2 .* (3 + 10 * r .^ 2 + 3 * r .^ 4) .^ (-0.1e1 / 0.2e1)) / 0.2e1;
