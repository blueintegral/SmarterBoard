...
% end
gss = gaussianss_doh_3D(I,min_scale,max_scale,S,sigma0);
[M,N,T,S]=size(gss.doH);
idx = localmax_3D(gss.doH, 1e-7) ;
idx = [idx localmax_3D(-gss.doH,-1e-7)];
[i,j,k,s] = ind2sub( size( gss.doH ), idx ) ;
oframes = [j(:)';i(:)';k(:)';s(:)'] ;
oframes(1:4,:) = oframes(1:4,:) - 1;
oframes = refinemx_3D(oframes,gss.doH) ;
oframes(1:4,:) = oframes(1:4,:) + 1;
...
zr=cordr(3,:);
sr=cordr(4,:);
ind1=sub2ind(size(gss.doH),y1,x1,z1,s1);
ind2=sub2ind(size(gss.doH),y1,x1,z2,s1);
ind3=sub2ind(size(gss.doH),y2,x1,z1,s1);
ind4=sub2ind(size(gss.doH),y2,x1,z2,s1);
ind5=sub2ind(size(gss.doH),y1,x2,z1,s1);
ind6=sub2ind(size(gss.doH),y1,x2,z2,s1);
ind7=sub2ind(size(gss.doH),y2,x2,z1,s1);
ind8=sub2ind(size(gss.doH),y2,x2,z2,s1);
ind9=sub2ind(size(gss.doH),y1,x1,z1,s2);
ind10=sub2ind(size(gss.doH),y1,x1,z2,s2);
ind11=sub2ind(size(gss.doH),y2,x1,z1,s2);
ind12=sub2ind(size(gss.doH),y2,x1,z2,s2);
ind13=sub2ind(size(gss.doH),y1,x2,z1,s2);
ind14=sub2ind(size(gss.doH),y1,x2,z2,s2);
ind15=sub2ind(size(gss.doH),y2,x2,z1,s2);
ind16=sub2ind(size(gss.doH),y2,x2,z2,s2);

indr=sub2ind(size(gss.doH),yr,xr,zr,sr); %20130606
x20=x2-x;
y20=y2-y;
z20=z2-z;
s20=s2-s;
...
doH=gss.doH(ind1).*m1 + gss.doH(ind2) .* m2 + gss.doH(ind3) .* m3 + gss.doH(ind4) .* m4 + ...
    gss.doH(ind5) .* m5 + gss.doH(ind6) .* m6 + gss.doH(ind7) .* m7 + gss.doH(ind8) .* m8 + ...
    gss.doH(ind9) .* m9 + gss.doH(ind10) .* m10 + gss.doH(ind11) .* m11 + gss.doH(ind12) .* m12 + ...
    gss.doH(ind13) .* m13 + gss.doH(ind14) .* m14 + gss.doH(ind15) .* m15 + gss.doH(ind16) .* m16;
...
for ii=1:length(Lxx)
    ...
    c = (-1)^(D(1)>0)*abs(doH(ii) ^ (0.1e1 / 0.3e1) * ((1 + H) * (1 + H * K) * (1 + G * H * K)) ^ (0.5e1 / 0.6e1) * G ^ (-0.1e1 / 0.2e1) * H ^ (-0.3e1 / 0.2e1) / K);
    if abs(c)<thresh
        continue;
    end
